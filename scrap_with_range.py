...
folders_range = ["0.0"] + ["0."+str(i) for i in range(4, 10)] + ["100"]

# use command line to download images
import os
os.chdir('./Scrap ISIC')

for start, end in zip(folders_range[:-1], folders_range[1:]):
    path = outdir + '/' + str(start) + "_to_" + str(end) + "mm/"

    if not os.path.exists(path):
        os.makedirs(path)

    # get all images with the same thickness
    df = df[df['mel_thick_mm'] >= float(start)]
    images = df[df['mel_thick_mm'] < float(end)]
